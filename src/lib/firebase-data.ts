...
async function ensureFirebaseUserDocument(user: User) {
  const ref = doc(firestore, "users", user.uid);
  const existing = await getDoc(ref);
  const fallbackName = user.displayName ?? "Kasutaja";
  const fallbackEmail = user.email ?? "";

  if (!existing.exists()) {
    await setDoc(ref, { name: fallbackName, email: fallbackEmail, createdAt: serverTimestamp() });
  }
...
function isAndroidDeveloperError(error: unknown) {
...
export async function loginWithGoogle() {
  if (Capacitor.isNativePlatform()) {
    try {
      ...
      if (!idToken)
        throw new Error(
          "Google’i sisselogimise token puudub. Kontrolli Androidi Google OAuth seadistust.",
        );
      const credential = await signInWithCredential(
        firebaseAuth,
        GoogleAuthProvider.credential(idToken),
      );
      return ensureFirebaseUserDocument(credential.user);
    } catch (error) {
      if (isAndroidDeveloperError(error)) {
        throw new Error(
          "Google’i sisselogimise Androidi seadistus ei klapi Firebase’iga. Kontrolli, et Firebase Android app package name, SHA-1/SHA-256 ja värske google-services.json vastaksid sellele buildile.",
        );
      }
      throw error;
    }
