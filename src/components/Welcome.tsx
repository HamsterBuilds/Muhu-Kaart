...
export default function Welcome({ onReady }: { onReady: (user: User) => void }) {
  ...
  const googleSubmit = async () => {
    ...
    try {
      ...
      toast.error(e instanceof Error ? e.message : "Google’i sisselogimine ebaõnnestus");
    } finally {
      setBusy(false);
    }
  };
        ...
        <div className="w-full max-w-md space-y-6">
          <header className="space-y-2 text-center">
            ...
            <p className="text-sm text-muted-foreground">
              Grupiga liitumine on vajalik, et näha ja jagada Muhu punkte.
            </p>
          </header>
          <div className="space-y-4">
            ...
            <input
              ...
              inputMode="numeric"
              placeholder="6-kohaline grupikood"
              className="w-full rounded-xl border border-input bg-background px-4 py-3 tracking-[0.2em] outline-none focus:border-accent"
            />
            <button
              disabled={
                groupBusy ||
                (groupName.trim().length < 2 && joinCode.length !== 6) ||
                (groupName.trim().length > 0 && joinCode.length === 6)
              }
              onClick={async () => {
                setGroupBusy(true);
                try {
                  if (groupName.trim()) await createFirebaseGroup(groupName);
                  else await joinFirebaseGroup(joinCode);
              ...
              className="w-full rounded-xl bg-primary px-4 py-4 text-base font-semibold text-primary-foreground disabled:opacity-40"
            >
              {groupBusy
                ? "Hetk..."
                : groupName.trim()
                  ? "Loo grupp ja jätka"
                  : "Liitu grupiga ja jätka"}
            </button>
            <p className="text-center text-xs text-muted-foreground">
              Sisesta ainult üks võimalus: loo grupp või kasuta olemasoleva grupi koodi.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="relative flex min-h-dvh flex-col items-center overflow-hidden bg-[#c7ecfb] text-center">
        ...
        <div className="relative flex min-h-dvh w-full max-w-md flex-col items-center px-6 pt-7">
          ...
          <button
            ...
            className="relative mb-7 w-[78%] max-w-sm rounded-full bg-[#292729] px-6 py-5 text-xl font-semibold text-white shadow-lg transition-transform hover:scale-[1.01] active:scale-[0.98]"
          >
            Alusta
          </button>
          <p className="relative mb-8 text-lg font-medium text-[#242224]">Roberti poolt</p>
        </div>
        <div className="relative flex h-20 w-full items-center justify-center bg-[#292729] text-lg font-semibold text-white">
          Muhu kaart
        </div>
      </div>
    );
        ...
        {!emailMode && (
          <div className="space-y-3">
            <button
              ...
              className="mx-auto flex h-28 w-full items-center justify-center gap-3 rounded-[2rem] bg-white px-5 text-xl font-semibold text-[#202124] shadow-sm transition-transform hover:scale-[1.01] active:scale-[0.99] disabled:opacity-40"
            >
            ...
            <button
              ...
              className="mx-auto block h-28 w-full rounded-[2rem] bg-white px-5 text-xl font-semibold text-[#202124] shadow-sm transition-transform hover:scale-[1.01] active:scale-[0.99]"
            >
              Logi sisse emailiga
            </button>
          </div>
